...
int main(void) {
  ...
  RunGame(&player, &game_audio, &game_settings, &game_scene, &game_interactive,
          &intro_dialogue, &game_map, worldObjects, &game_context);
...
void InitGame(Settings *game_settings) {
  ...
  // Prevent closing the window with ESC automatically so it can be used for
  // pause.
  SetExitKey(0);
}
...
void RunGame(Character *player, Audio *game_audio, Settings *game_settings,
  ...
  Dialogue *current_dialogue =
      intro_dialogue; // Pointer to the currently active dialogue (if any)
  Interactable *objectToInteractWith = NULL;

  while (!WindowShouldClose()) {
    // Update audio stream.
    UpdateAudio(game_audio);
    Rectangle playerHitbox = {player->position.x + 50, player->position.y + 50,
                              80, 80};
    ...
    if (game_state == PAUSE) {
      DrawText("PAUSED",
               game_settings->window_width / 2 - MeasureText("PAUSED", 40) / 2,
               game_settings->window_height / 2 - 20, 40, LIGHTGRAY);
    }
